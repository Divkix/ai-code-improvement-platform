...
<script lang="ts">
    ...
    async function loadRepositoryData() {
        ...
        try {
            ...
            if (!repoResponse.ok) {
                throw new Error(`Failed to load repository: ${repoResponse.status} ${repoResponse.statusText}`);
            }
...
<svelte:head>
    <title>{repository ? `Search in ${repository.name}` : 'Repository Search'} - GitHub Analyzer</title>
    <meta name="description" content={repository ? `Search code in ${repository.fullName}` : 'Search repository code'} />
</svelte:head>
...
<div class="repository-search-page">
    ...
    {:else if repositoryError}
        <div class="repository-error">
            ...
            <p>{repositoryError}</p>
            <button class="retry-button" on:click={loadRepositoryData}>
                Try Again
            </button>
        </div>
    {:else if repository}
        ...
        <div class="search-container">
            ...
            <SearchBox
                ...
                on:clear={() => { searchQuery = ''; searchResults = null; }}
            />
