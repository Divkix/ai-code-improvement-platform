<script lang="ts">
	...
	async function loadRecentRepositories() {
		...
		try {
			...
			hasMore = false; // Recent repos don't use pagination
		} catch (err) {
			error = err instanceof Error ? err.message : 'Failed to load recent GitHub repositories';
			console.error('Error loading recent GitHub repositories:', err);
		} finally {
			loading = false;
		}
	}
	...
	async function performSearch(query: string) {
		if (!user.githubConnected || !query.trim()) {
		...
		try {
			...
			hasMore = false; // Search results don't use pagination
		} catch (err) {
			error = err instanceof Error ? err.message : 'Failed to search GitHub repositories';
...
{:else}
	...
	<div class="mb-4">
		<div class="relative">
			...
			<Input
				type="text"
				placeholder="Search repositories (e.g., 'react', 'user/repo')..."
		...
		{#if !searchQuery}
			<p class="mt-2 text-xs text-muted-foreground">
				Showing your 6 most recently updated repositories. Start typing to search.
			</p>
